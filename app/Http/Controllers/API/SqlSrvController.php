...
use App\Model\AdvertizeProduct;
            ...
            if ($user) {
               if ($setting->customer_reg_email == '1' && $user->NTT_ENTITY_STATUS == 0) {
                  ...
                  if (!$gettoken) {
                     ...
                     $store->user_id = $user->NTT_ENTITY_ADVERTIZER_ID;
                     $store->save();
                  } else {
                     $gettoken->user_id = $user->NTT_ENTITY_ADVERTIZER_ID;
                     $gettoken->save();
                  }
                  if ($user->NTT_BUSINESS_ADDRESS == null) {
                     $user->NTT_BUSINESS_ADDRESS = "";
                  }
                  if ($user->NTT_BUSINESS_ADDRESS == null) {
                     $user->NTT_BUSINESS_ADDRESS = "";
                  }
                  if ($user->NTT_ADVERTIZER_NAME == null) {
                     $user->NTT_ADVERTIZER_NAME = "";
                  }
                  if ($user->NTT_BUSINESS_ADDRESS == null) {
                     $user->NTT_BUSINESS_ADDRESS = "";
                  }
                  if ($user->NTT_MOBILE == null) {
                     $user->NTT_MOBILE = "";
                  }
                  $cartdata=CartData::on('sqlsrv')->where("user_id",$user->NTT_ENTITY_ADVERTIZER_ID)->get();
                  $wishdata=Wishlist::on('sqlsrv')->where("user_id",$user->NTT_ENTITY_ADVERTIZER_ID)->get();
                  $user->cartdata=count($cartdata);
                  $user->wishdata=count($wishdata);
...
//    SELECT TOP (1000) [ADD_ADVERTIZE_ID]
//       ,[ADD_TITLE]
//       ,[ADD_NTT_ENTITY_ADVERTIZER_ID],[ADS_ADVERTIZE_SECTION_ID]
//       ,[ADS_COT_COUNTY_ID]
//       ,[ADS_PRV_PROVINCE_ID]
//       ,[ADS_CNT_COUNTRY_ID]
//       ,[ADS_NEA_NEAREST_AREA_ID]
//       ,[ADD_ADT_ADVERTIZE_TYPE]
//       ,[ADS_CRY_CURRENCY_ID]
//       ,[ADS_ABT_ADVERTIZE_BY_TYPE]
//       ,[ADS_DESCRIPTION]
//       ,[ADS_STATUS]
//       ,[ADS_SHOW_STATUS]
//       ,[ADS_CREATE_DATE]
...
//       ,[ADS_SOLD_DATE]
//       ,[ADS_YOUTUBE]
//       ,[ADS_IS_AGREEDWITHTERMSCONDITIONS]
//       ,[ADS_BUMP_TIMES]
//       ,[ADS_BUMP_DATE]
//       ,[ADS_FEATURE_DATE]
//       ,[ADS_FEATURE_TIMES]
//       ,[ADS_ADVERTIZE_PARENT_ID]
//       ,[ADS_TEMP_NAME]
//       ,[ADS_IS_PHONE]
//       ,[ADS_IS_MSG]
//       ,[ADS_TEMP_PHONE]
...
//       ,[ADS_AllCats]
//       ,[ADS_PRICE]
//   FROM [RiteBuy2016DevDB].[dbo].[Tbl_AdvertizeMaster]
   ...
   public function userregister(Request $request)
   {
      ...
      if ($validator->fails()) {
         ...
         if (count($user) == 0) {
            ...
            $response = array("status" => 1, "msg" => "Register Successfully", "data" => $userNtt);
         } else if (count($user) != 0) {
            $response = array("status" => 0, "msg" => "User Already Registered");
         } else {
   ...
   {
      //TODO: How to validate Ads? current ADS_STATUS = 2 
      $data = DB::connection('sqlsrv')->table('Tbl_AdvertizeMaster')
         ->select('ADD_ADVERTIZE_ID as product_id', DB::connection('sqlsrv')->raw('count(*) as total'))
         ->orderby('total', 'DESC')
         ->where('Tbl_AdvertizeMaster.ADS_STATUS', '2')
         ->groupBy('ADD_ADVERTIZE_ID')
         ->paginate(10);
      
         // $data = DB::connection('sqlsrv')
         // ->select('SET NOCOUNT ON exec [RiteBuy2016DevDB].[dbo].[SPShowAdsOnIndexPage]', DB::connection('sqlsrv')->raw('count(*) as total'))
         // ->orderby('total', 'DESC')
         // ->paginate(10);
         ...
         // ::hybrate
         // FetchAll()
      //$data = DB::connection('sqlsrv')->select('SET NOCOUNT ON exec [RiteBuy2016DevDB].[dbo].[SPShowAdsOnIndexPage]');
      // // foreach ($data as $Ads) {
      // // var_dump($Ads->ADD_TITLE);
      // // }
      
      // var_dump($data);
      // exit();

      // var_dump(count($checkExistingWebUser));
      // echo($checkExistingWebUser->ADD_ADVERTIZE_ID);
      // exit();
      ...
      if (count($data) != 0) {
         foreach ($data as $k) {
            ...
            $getproduct = AdvertizeProduct::on('sqlsrv')->where("ADD_ADVERTIZE_ID", $k->product_id)->first();
            if ($getproduct) {
               // $avgStar = Review::where("product_id", $k->product_id)->avg('ratting');
               // $wish = Wishlist::on('sqlsrv')->where("product_id", $k->product_id)->where("user_id", $user_id)->first();
               // echo($wish); //converting the nvarchar value (ADD_ADVERTIZE_ID) to int
               // exit();
               // $total = Review::where("product_id", $k->product_id)->get();
               
               $dat['id'] = $getproduct->ADD_ADVERTIZE_ID;
               $dat['name'] = $getproduct->ADD_TITLE;
               $dat['image'] = asset('public/upload/product/') . '/' . $getproduct->GRL_CHANGED_IMAGE_NAME;
               // $dat['MRP'] = $Ads->MRP;
               // $dat['ratting'] = round($avgStar);
               // $dat['totalreview'] = count($total);
               $dat['price'] = $getproduct->ADS_PRICE;
               // $dat['wish'] = count($wish);
               // $dat['discount'] = $Ads->discount;
               $k->productdetail = $dat;
         ...
         $total = Wishlist::on('sqlsrv')->where("user_id", $user_id)->get();
         $cartdata = CartData::on('sqlsrv')->where("user_id", $user_id)->get();
         // echo "<pre>";var_dump($cartdata);exit;
         $response = array(
            'status' => 1,
