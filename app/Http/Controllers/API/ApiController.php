...
class ApiController extends Controller {
 ...
 public function file_get_contents_curl($url) {
   ...
   public function testconnection(){
    //phpinfo();exit;
     //$serverName = "tfml-server.database.windows.net, 1433"; //serverName\instanceName, portNumber (default is 1433)
    $serverName = "77.68.125.42, 1433";

        $conn = new \PDO("sqlsrv:Server=tfml-server.database.windows.net;Database=TFML", "TheodoreFront", "Schikanedr_tf#1");
     //$conn = new \PDO("sqlsrv:77.68.125.42;Database=OnClick", "sa", "Ritebuy$123");

        //$connectionInfo = array( "Database"=>"TFML", "UID"=>"TheodoreFront", "PWD"=>"Schikanedr_tf#1");
     $connectionInfo = array( "Database"=>"OnClick", "UID"=>"sa", "PWD"=>"Ritebuy$123");
//$conn = sqlsrv_connect( $serverName, $connectionInfo);

        if( $conn ) {
            echo "Connection established.<br />";

            $sql = "SELECT count(*) FROM VIEW_SHOPIFY_INV ;";
